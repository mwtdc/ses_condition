...
""" Задаем переменные путей (если папка есть, то чистим, если нет, то создаем) и прочее """
""" Проверяем запущен .py файл или .exe и в зависимости от этого получаем пути """
if getattr(sys, 'frozen', False):
    windy_path=str(pathlib.Path(sys.executable).parent.absolute())+'/windy/'
    ...
    cam_path=str(pathlib.Path(sys.executable).parent.absolute())+'/cam/'
    ...
    result_path=str(pathlib.Path(sys.executable).parent.absolute())+'/result/'
    ...
    nosignal_image=str(pathlib.Path(sys.executable).parent.absolute())+'/nosignal.jpg'
    gecko_path = str(pathlib.Path(sys.executable).parent.absolute())+'/geckodriver.exe'
    firefox_path = str(pathlib.Path(sys.executable).parent.absolute())+'/FirefoxPortable/App/Firefox64/firefox.exe'
    settings = str(pathlib.Path(sys.executable).parent.absolute())+'/settings.ini'
elif __file__:
    windy_path=str(pathlib.Path(__file__).parent.absolute())+'/windy/'
    if not os.path.exists(windy_path): os.makedirs(windy_path) 
    list(map(os.unlink, (os.path.join(windy_path,f) for f in os.listdir(windy_path))))
    cam_path=str(pathlib.Path(__file__).parent.absolute())+'/cam/'
    if not os.path.exists(cam_path): os.makedirs(cam_path) 
    list(map(os.unlink, (os.path.join(cam_path,f) for f in os.listdir(cam_path))))
    result_path=str(pathlib.Path(__file__).parent.absolute())+'/result/'
    if not os.path.exists(result_path): os.makedirs(result_path)
    list(map(os.unlink, (os.path.join(result_path,f) for f in os.listdir(result_path))))
    parent_path=str(pathlib.Path(__file__).parent.absolute())
    nosignal_image=str(pathlib.Path(__file__).parent.absolute())+'/nosignal.jpg'
    gecko_path = str(pathlib.Path(__file__).parent.absolute())+'/geckodriver.exe'
    firefox_path = str(pathlib.Path(__file__).parent.absolute())+'/FirefoxPortable/App/Firefox64/firefox.exe'
    settings = str(pathlib.Path(__file__).parent.absolute())+'/settings.ini'
...
""" Настройки для драйвера Firefox (скрытый режим и установка драйвера(закоменчена),
 теперь берется geckodriver.exe из этой же папки и portable версия firefox (чтобы работало даже на чистой системе)) """
options = Options()
...
with connection.cursor() as cursor:
    """ Условие если текущее время меньше 9 часов,то работаем по второй ценовой зоне, если больше, то по первой. """
    ...
    cursor.execute(sql)    
    ses_url=pd.DataFrame(cursor.fetchall(), columns=['ses','ses_eng','cam','windy'])
    connection.close()
    ses_url['dt']=0
    ses_url.head()
for windy_url in range(len(ses_url)):
    ...
    sleep(5) 
    WebDriverWait(browser, 10).until(EC.presence_of_element_located((By.CLASS_NAME, 'fg-red.size-xs.inlined.clickable')))
    """ click_button = browser.find_element_by_class_name('fg-red.size-xs.inlined.clickable').click() """
    click_button = browser.find_element_by_class_name('fg-red.size-xs.inlined.clickable')
    browser.execute_script("arguments[0].click();", click_button)
    sleep(1) 
    """ Найдем высоту таблицы с погодой (если она меньше 269.5, то пробуем обновить страницу)(на случай непрогруза страницы) """
    HeightElement = browser.find_element_by_id('detail-data-table').size['height']
    if HeightElement < 200:
        ...
        browser.execute_script("arguments[0].click();", click_button)
        sleep(1)  
    featureElement = browser.find_element_by_id('detail-data-table').screenshot_as_png
    """ Найдем ширину таблицы с погодой(меняется от времени просмотра прогноза) """
    WidthElement = browser.find_element_by_id('detail-data-table').size['width']
    imageStream = io.BytesIO(featureElement)
    im = Image.open(imageStream)
    im.save(windy_path + ses_name_eng + '.png')
        ...
        if code_url == 200:
            ...
            if t.is_alive():
                ...
                if cap.isOpened():
                    ret, frame = cap.read()
                    cap.release() 
                    """ Если камера не работает (нет потока видео), то брать картинку "нет сигнала" """
                    if ret is False:
                        frame = cv2.imread(nosignal_image) 
                else:
                    if cap is None:
    ...
    writestatus=cv2.imwrite(cam_path + ses_name_eng + '.png', frame, [int(cv2.IMWRITE_PNG_COMPRESSION), 9])
    """ Склеиваем 2 скрина """
    img1 = cv2.imread(cam_path + ses_name_eng + '.png')
    img2 = cv2.imread(windy_path + ses_name_eng + '.png')
    vis = np.concatenate((img1, img2), axis=0)
    """ Добавляем на скрин с камеры название СЭС """
    cv2.rectangle(vis, (0,0), (900,80), (255,255,255), -1)
    cv2.putText(vis, ses_name, (10,50), cv2.FONT_HERSHEY_COMPLEX, 2, (0,0,0), 2)
    """ Сохраняем итоговую картинку со сжатием """
    cv2.imwrite(result_path + ses_name_eng + '.png', vis, [int(cv2.IMWRITE_PNG_COMPRESSION), 9])
    """ Конвертируем итоговую png в pdf(нужно для создания итогового pdf с закладками) """
    with open(result_path + ses_name + '.pdf', "wb") as f:
        f.write(img2pdf.convert(glob.glob(result_path + ses_name_eng + '.png')))
browser.quit()
browser.__exit__()
...
if time_cutoff > datetime.now().hour:
    output_filename='ses_condition_sib_'+time_for_name+'.pdf'
else:
    output_filename='ses_condition_eur_'+time_for_name+'.pdf'
...
def mergefiles(path, output_filename, import_bookmarks=False):
    ...
    if len(filelist) == 0:
        print("There is no pdf file in the current directory and subdirectories")
        sys.exit()
...
updater.bot.send_document(chat_id=channel_id, document=open(result_path+output_filename, 'rb'), timeout=100.0)
